...
class BiddingConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        await self.accept()
        logger.info(
            "WebSocket connected: round_id=%s channel=%s",
            self.round_id,
            self.channel_name,
        )
    ...
    async def receive(self, text_data):
        try:
            ...
            logger.warning(
                "Invalid JSON received on round_id=%s: %s",
                self.round_id,
                text_data[:200],
            )
            return
        ...
        if message_type == "new_bid":
            ...
            await self.channel_layer.group_send(
                self.room_group_name, {"type": "bid_update", "bid": data.get("bid")}
            )
        else:
            logger.warning(
                "Unknown message type '%s' on round_id=%s", message_type, self.round_id
            )

    async def bid_update(self, event):
        await self.send(
            text_data=json.dumps({"type": "bid_update", "bid": event["bid"]})
        )

...
import { receiptOutline, trophy } from "ionicons/icons";
...
const HistoryCard: React.FC<HistoryCardProps> = ({ round, onClick, isEmpty = false }) => {
    // 1. Empty State
    if (isEmpty || !round) {
        return (
            <div className="history-empty-state">
                ...
                <p className="empty-subtitle">When rounds are completed, their transaction records will appear here.</p>
            </div>
        );
    }
    ...
    return (
        <div className="history-card" onClick={onClick}>
            ...
            <div className="history-details">
                ...
                <p className="history-date">{new Date(round.scheduled_time).toLocaleDateString()}</p>
            </div>
        </div>
    );
};

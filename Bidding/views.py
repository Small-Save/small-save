...
def bidding_room(request, round_id):
    ...
    try:
        member: GroupMember = GroupMember.objects.get(
            group=bidding_round.group, user=request.user
        )
...
def place_bid(request, round_id):
    ...
    if not bidding_round.is_active():
        logger.warning(
            "Bid rejected: round_id=%s is not active (status=%s)",
            round_id,
            bidding_round.status,
        )
        return CustomResponse(
            is_success=False,
            error="Bidding is not active",
    ...
    try:
        ...
        logger.warning(
            "Bid rejected: user_id=%s is not a member of group for round_id=%s",
            request.user.id,
            round_id,
        )
        return CustomResponse(
    ...
    if member.has_won:
        logger.warning(
            "Bid rejected: user_id=%s already won a round in group_id=%s",
            request.user.id,
            bidding_round.group_id,
        )
        return CustomResponse(
            is_success=False,
            error="Already won a round",
            status_code=status.HTTP_403_FORBIDDEN,
        )
    ...
    try:
        ...
        logger.warning(
            "Bid rejected: invalid amount from user_id=%s for round_id=%s",
            request.user.id,
            round_id,
        )
        return CustomResponse(
    ...
    if bidding_round.bids.filter(amount=bid_amount).exists():
        logger.info(
            "Bid rejected: duplicate amount=%s for round_id=%s by user_id=%s",
            bid_amount,
            round_id,
            request.user.id,
        )
    ...
    try:
        with transaction.atomic():
            bid = Bid.objects.create(
                bidding_round=bidding_round, member=member, amount=bid_amount
            )
    ...
    logger.info(
        "Bid placed: bid_id=%s amount=%s round_id=%s user_id=%s",
        bid.id,
        bid_amount,
        round_id,
        request.user.id,
...
def get_all_bids(request, round_id):
    ...
    if not GroupMember.objects.filter(
        ...
        logger.warning(
            "Bids access denied: user_id=%s not a member for round_id=%s",
            request.user.id,
            round_id,
        )
        return CustomResponse(
    ...
    logger.debug(
        "Fetched %d bids for round_id=%s by user_id=%s",
        len(bids),
        round_id,
        request.user.id,
    )
...
def start_bidding(request, round_id):
    ...
    if not bidding_round.can_start():
        ...
        return CustomResponse(
            error="Cannot start bidding yet", status_code=status.HTTP_400_BAD_REQUEST
        )
    ...
    logger.info(
        "Bidding started: round_id=%s group_id=%s by user_id=%s",
        round_id,
        bidding_round.group_id,
        request.user.id,
    )
...
def end_bidding(request, round_id):
    ...
    if not bidding_round.is_active():
        ...
        return CustomResponse(
            error="Bidding round is not active", status_code=status.HTTP_400_BAD_REQUEST
        )

    if not bidding_round.end_bidding():
        logger.warning(
            "Cannot end bidding: no eligible winner for round_id=%s", round_id
        )
        return CustomResponse(
            error="No eligible members to select a winner",
            status_code=status.HTTP_400_BAD_REQUEST,
        )
